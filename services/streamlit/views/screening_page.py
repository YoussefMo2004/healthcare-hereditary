...
def _seed_screening_data(conn: sqlite3.Connection) -> None:
    ...
    for (pid,) in patients:
        ...
        for i in range(num_preds):
            ...
            if score < 0.25: tier = "low"
            elif score < 0.50: tier = "moderate"
            elif score < 0.75: tier = "high"
            else: tier = "very_high"
            
            logs.append((
                str(uuid.uuid4()), pid, round(score, 4), tier, 
                "hereditary-risk-xgboost", "1.0", pred_date.strftime("%Y-%m-%d"),
                "batch", pred_date.isoformat()
            ))

    conn.executemany(
        "INSERT INTO prediction_log (id, patient_id, risk_score, risk_tier, model_name, model_version, feature_date, source, predicted_at) VALUES (?,?,?,?,?,?,?,?,?)",
        logs
    )
    conn.commit()
...
def render_screening_page() -> None:
    ...
    with tab_batch:
        ...
        with col1:
            target = st.radio("Target Population", ["All Active Patients", "Specific Gender", "Age Range"])
        ...
        with col2:
            st.info("Batch screening runs asynchronously and logs results to the PredictionLog for historical tracking.")
            run_btn = st.button("▶️ Start Batch Screening", type="primary", use_container_width=True)
            ...
            with st.spinner("Executing batch predictions..."):
                ...
                if pts.empty:
                    ...
                    for _, row in pts.iterrows():
                        ...
                        logs.append((
                            str(uuid.uuid4()), row["id"], score, tier, 
                            "hereditary-risk-xgboost", "1.0", feat_date,
                            "batch", now_str
                        ))
                    ...
                    # Color coding
                    def color_tier(val):
                        colors = {'low': 'green', 'moderate': 'orange', 'high': 'red', 'very_high': 'darkred'}
                        return f'color: {colors.get(val, "black")}'
                        
                    st.dataframe(res_df.style.applymap(color_tier, subset=['Tier']), use_container_width=True, hide_index=True)
    ...
    with tab_trends:
        ...
        selected_pid = st.selectbox("Select Patient to View Trends", options=list(p_options.keys()), format_func=lambda x: p_options[x], key="trend_select")

        if selected_pid:
            history_df = pd.read_sql_query(
                "SELECT predicted_at, risk_score, risk_tier FROM prediction_log WHERE patient_id = ? ORDER BY predicted_at ASC",
            ...
            if history_df.empty:
                ...
                if len(history_df) >= 2:
                    ...
                    m2.metric("Current Tier", history_df.iloc[-1]["risk_tier"].replace("_", " ").title())
                ...
                # Chart
                fig = px.line(
                    history_df, x="predicted_at", y="risk_score", 
                    markers=True, title="Longitudinal Risk Trajectory",
                    labels={"predicted_at": "Date", "risk_score": "Probability"}
                )
                fig.update_yaxes(range=[0, 1.0], tickformat=".0%")
                
                # Add risk tier bands
                fig.add_hrect(y0=0, y1=0.25, fillcolor="green", opacity=0.1, layer="below", line_width=0)
                fig.add_hrect(y0=0.25, y1=0.5, fillcolor="orange", opacity=0.1, layer="below", line_width=0)
                fig.add_hrect(y0=0.5, y1=0.75, fillcolor="red", opacity=0.1, layer="below", line_width=0)
                fig.add_hrect(y0=0.75, y1=1.0, fillcolor="darkred", opacity=0.1, layer="below", line_width=0)
