...
class TestDeidentification:
    ...
    def test_nested_dict_recursion(self) -> None:
        from libs.common.deidentification import deidentify_patient
        record: dict[str, Any] = {
            "metadata": {"email": "x@y.com", "score": 0.8}
        }
    ...
    def test_list_of_dicts_recursion(self) -> None:
        from libs.common.deidentification import deidentify_patient
        record: dict[str, Any] = {
            "relatives": [{"first_name": "Bob"}, {"first_name": "Carol"}]
        }
        result = deidentify_patient(record)
...
class TestRBAC:
    def test_admin_has_all_permissions(self) -> None:
        from services.api.auth.rbac import Permission, Role, ROLE_PERMISSIONS
    ...
    def test_service_role_mirrors_clinician_permissions(self) -> None:
        from services.api.auth.rbac import Role, ROLE_PERMISSIONS
        assert ROLE_PERMISSIONS[Role.SERVICE] == ROLE_PERMISSIONS[Role.CLINICIAN]
